...
class Masterdistributors extends CI_Controller {
	...
	function __construct() {
        ...
        $this->load->model('masterdistributor');       
    ...
    public function index(){   
        ...
        // Validate User Auth.        
        if(!CheckPermission(MASTER, "own_read")){
       ...
       //get messages from the session
        ...
        $this->load->view('masterdistributor/list', array());
    ...
    public function get_tables() {
        ...
        $rows = $this->masterdistributor->getRow();  
        }else {
           $rows = $this->masterdistributor->getRowFilter($this->session->userdata('userId'));  
        }
        $mem = array();  
        foreach($rows->result() as $r) {   
            if(CheckPermission(MASTER, "own_update")){        
            $mman = '<a href="master-distributors-users/edit/'.$r->id.'" data-toggle="tooltip" title="Edit" class="glyphicon glyphicon-edit" style="padding-right:5px;"></a> ';
            }else {
             $mman = '';
            }
            if(CheckPermission(MASTER, "own_delete")){    
            $mman1 = '<a href="master-distributors-users/delete/'.$r->id.'" data-toggle="tooltip" title="Delete"  class="glyphicon glyphicon-trash" onclick="return confirm(\'Are you sure to delete?\')" ></a>';
            }else {
             $mman1 = '';
            }
     ...
     */
     ...
     public function add()
     {
        ...
        // Validate User Auth.        
        if(!CheckPermission(MASTER, "own_create")){
            $this->session->set_userdata('error_msg', 'You are not authorized to access the page..!');
            redirect('dashboard');
        }
        ...
        if($this->input->post('userSubmit')){            
            ...
            $this->form_validation->set_message('is_unique', 'Email Already Exists, Please use a different email address..!');
             ...
             if($this->input->post('uid') == 2 || $this->input->post('uid') == 14){
                $user_type = 3;
            }else{
                $user_type = 3;
            }
           ...
           // $postData['isEmailVerified'] =  0;
            ...
            $postData['profile_pic'] =  $dataq['file_name'];
                if($this->form_validation->run() == true){
                    $insert = $this->masterdistributor->insert($postData);
                    if($insert){
                        if(setting_all('sms_alerts') == '1'){
                            sendSMS('Congratulation for being a masterdistributor, you will receive your login details on your registered email address. Thank You'.setting_all('company_name').'..!', $this->input->post('mobile'));
                        }
                        $this->session->set_userdata('success_msg', 'Master Distributor has been added successfully.');
                        redirect('master-distributors-users');
                    }else{
        ...
        //load the list page view
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/breadcrum', $data);
            $this->load->view('masterdistributor/add', $data);
            $this->load->view('templates/footer', $data);
    ...
    public function edit($id){
        ...
        if($data['user']['user_type'] == 1){
         $postData = $this->masterdistributor->getRowsa($id);
        }else {
         $postData = $this->masterdistributor->getRows($id, $data['user']['id']);
        }
        $data['statelist'] = $this->listall->getState(); 
        $data['countrylist'] = $this->listall->getCountry(); 
        $data['citylist'] = $this->listall->getCity(); 
        ...
        if($this->input->post('ememberSubmit')){
           ...
           // var_dump('$postData');
            if($this->form_validation->run() == true){
               ...
               $update = $this->masterdistributor->update($postData, $id);

                    if($update){
                        if(setting_all('sms_alerts') == '1'){
                        sendSMS('Your profile has been updated successfully. Thank You'.setting_all('company_name').'..!', $this->input->post('mobile'));
                        }
                        $this->session->set_userdata('success_msg', 'Master Distributor has been updated successfully.');
                        redirect('master-distributors-users');
                    }else{
        ...
        //load the list page view
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/breadcrum', $data);
            $this->load->view('masterdistributor/edit', $data);
            $this->load->view('templates/footer', $data);
    }
    ...
    public function view($id){       
        ...
        if(!empty($id)){
            $dataa['member'] = $this->masterdistributor->getRows($id);
            
            //load the list page view
            $this->load->view('templates/header', $data);
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/breadcrum', $data);
            $this->load->view('masterdistributor/view', $dataa);
            $this->load->view('templates/footer', $data);
        }else{
            redirect('master-distributors-users');
        }
    ...
    public function delete($id){
        ...
        if($id){
            //delete post
            $delete = $this->masterdistributor->deletes($id);            
            if($delete){
    ...
    public function fund(){   
        ...
        $postData = array();
        $data['user'] = $this->user->getRows(array('id'=>$this->session->userdata('userId')));
        $data['wllist'] = $this->masterdistributor->getwlUsers($this->session->userdata('userId')); 
       ...
       //get messages from the session
            ...
            if($this->form_validation->run() == true){ 
                ...
                if($uid != "1"){
                        if($postData['amount'] > $postData['senderbalance'] && $postData['paymentType'] == 'Credit'){
                            ...
                            redirect('master-distributors-users/fund');
                        }else {
        ...
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/breadcrum', $data);
        $this->load->view('masterdistributor/fund', $data);
        $this->load->view('templates/footer', $data);
