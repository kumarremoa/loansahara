...
class Landline extends CI_Controller {
	...
	public function index()
	{
        ...
        // Validate User Auth.        
        if(!CheckPermission(DTH, "own_read")){
       ...
       //get messages from the session
        ...
        $this->load->view('services/landline', $data);
	...
	function getOperator(){
		...
		$postfields=json_encode(array("type_id"=>"9"));
	...
	function llrecharge(){
		...
		$ubal=$this->input->post('ubal');

		$arr=json_encode(array("operator"=>$operator, "account"=>$account, "landline_no"=>$landline_no, "amount"=>$amount, "std_code"=>$std_code));
		$postfields=$arr;
		
		$url=api_url().'recharge/landlinerecharge';	  
	    $response=curl_function($postfields,$url);
	    $response = json_decode($response, true);
	 //   print_r($response);// die;
	  ...
	  /* $response = Array("statusCode" => "200", "message" => "Transaction Pending", "data" => Array("txnid" => "218778079", "request_id" => "123dcbedb125", "oper_id" => "4913047241308170139"));*/
	    ...
	    if( $response['statusCode'] == 200){
	    	$deduction = $this->recharge->reducelandlineFund($userid, $amount, $landline_no);
	    	...
	    	if($deduction){
	    	 $message =  '<div class="col-md-4"></div>  
		               	<div class="col-md-4 table-bordered" >
			               		...
			               		</div><div class="clearfix"></div><hr><input type="hidden" class="form-control" id="mobile" name="mobile" value="'.$landline_no.'">		               		
								...
								<p class="success">'.$response['message'].'<br><table class="table table-striped"><tr><td>Transaction ID :</td><td>'.$response['data']['txnid'].'</td></tr><tr><td>Landline No :</td><td>'.$std_code .'-'. $landline_no.'</td></tr><tr><td>Amount :</td><td>'.inr($amount).'</td></tr><tr><td>Operator Reference No :</td><td>'.$response['data']['oper_id'].'</td></tr></table></p>
	    	...
	    	$message =  '<div class="col-md-4"></div>  
		               	<div class="col-md-4 table-bordered" >
								...
								<p class="error">'.$response['message'].'<br><table class="table table-striped"><tr><td>Mobile No :</td><td>'.$landline_no.'</td></tr><tr><td>Amount :</td><td>'.inr($amount).'</td></tr></tr></table></p>
			               		</div><div class="col-md-4"></div>';
	    }
