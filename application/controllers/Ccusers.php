...
class Ccusers extends CI_Controller {
    ...
    public function index(){   
       ...
       //get messages from the session
        ...
        $this->load->view('ccusers/list', array());
    ...
    public function get_tables() {
        ...
        $rows = $this->ccuser->getRow();  
        }else {
        $rows = $this->ccuser->getRowFilter($this->session->userdata('userId'), $this->session->userdata('userType'));    
        }
        $mem = array();  
        foreach($rows->result() as $r) {   
            if(CheckPermission(CCUSERS, "own_update")){        
            $mman = '<a href="customer-care-users/edit/'.$r->id.'" data-toggle="tooltip" title="Edit" class="glyphicon glyphicon-edit" style="padding-right:5px;"></a> ';
            }else {
             $mman = '';
            }
            if(CheckPermission(CCUSERS, "own_delete")){    
            $mman1 = '<a href="customer-care-users/delete/'.$r->id.'" data-toggle="tooltip" title="Delete"  class="glyphicon glyphicon-trash" onclick="return confirm(\'Are you sure to delete?\')" ></a>';
            }else {
             $mman1 = '';
            }
     ...
     */
     ...
     public function add(){
        ...
        // Validate User Auth.        
        if(!CheckPermission(CCUSERS, "own_create")){
            $this->session->set_userdata('error_msg', 'You are not authorized to access the page..!');
            redirect('dashboard');
        }
           ...
           // $postData['isEmailVerified'] =  0;
            ...
            $postData['profile_pic'] =  $dataq['file_name'];
                if($this->form_validation->run() == true){
                    $insert = $this->ccuser->insert($postData);
                    if($insert){
        ...
        //load the list page view
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/breadcrum', $data);
            $this->load->view('ccusers/add', $data);
            $this->load->view('templates/footer', $data);
    ...
    public function edit($id){
        ...
        $data = array();
        $data['user'] = $this->user->getRows(array('id'=>$this->session->userdata('userId')));
        $postData = $this->ccuser->getRows($id);
        ...
        if($this->input->post('ememberSubmit')){
           ...
           // var_dump('$postData');
            if($this->form_validation->run() == true){
               ...
               $update = $this->ccuser->update($postData, $id);

                    if($update){
                        if(setting_all('sms_alerts') == '1'){
                        sendSMS('Your profile has been updated successfully. Thank You'.setting_all('company_name').'..!', $this->input->post('mobile'));
                        }
                        $this->session->set_userdata('success_msg', 'Customer Care has been updated successfully.');
                        redirect('customer-care-users');
                    }else{
        ...
        //load the list page view
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/breadcrum', $data);
            $this->load->view('ccusers/edit', $data);
            $this->load->view('templates/footer', $data);
    }
    ...
    public function view($id){       
        ...
        if(!empty($id)){
            $dataa['member'] = $this->ccuser->getRows($id);
            
            //load the list page view
            $this->load->view('templates/header', $data);
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/breadcrum', $data);
            $this->load->view('ccusers/view', $dataa);
            $this->load->view('templates/footer', $data);
        }else{
            redirect('customer-care-users');
        }
    ...
    public function fund(){   
       ...
       //get messages from the session
            ...
            if($this->form_validation->run() == true){ 
                ...
                if($uid != "1"){
                        if($postData['amount'] > $postData['senderbalance'] && $postData['paymentType'] == 'Credit'){
                            ...
                            redirect('customer-care-users/fund');
                        }else {
        ...
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/breadcrum', $data);
        $this->load->view('customer-care-users/fund', $data);
        $this->load->view('templates/footer', $data);
