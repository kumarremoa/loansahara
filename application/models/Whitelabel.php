...
class Whitelabel extends CI_Model {
    ...
    function getRows($id = ""){
        if(!empty($id)){            
            $query = $this->db->get_where('loan_users', array('id' => $id,'user_type' => '2'));
    ...
    function getRow(){
        ...
        $this->db->where('loan_users.user_type', '14');
        $this->db->or_where('loan_users.user_type', '2');
        $this->db->where('loan_users.is_deleted', '0');
    ...
    public function insert($data = array()) {
     ...
     public function insertsettings($data = array()) {
        $insert = $this->db->insert('loan_setting', $data);
        if($insert){
            return $this->db->insert_id();
        }else{
            return false;
...
/* End of file Whitelabel.php */
/* Location: ./application/models/Whitelabel.php */